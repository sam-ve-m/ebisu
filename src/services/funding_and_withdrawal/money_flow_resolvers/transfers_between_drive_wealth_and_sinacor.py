...
from src.repositories.funding_and_withdrawal.realtime.repository import RealtimeFundingAndWithdrawalRepository
from src.infrastructures.env_config import config


class TransfersBetweenDriveWealthAndSinacor(MoneyFlowResolverAbstract):
    ...
    async def _get_spread(self) -> float:
        ...
        spread = await self.funding_and_withdrawal_realtime_data_repository.get_tax(cash_conversion=cash_conversion_references)

        return spread

    async def _get_tax(self) -> float:
        cash_conversion_references = self._get_cash_conversion_references()
        tax = await self.funding_and_withdrawal_realtime_data_repository.get_tax(cash_conversion=cash_conversion_references)

        return tax

    async def _convert_value(self) -> float:
        cash_conversion_references = self._get_cash_conversion_references()
        conversion_base = await self.funding_and_withdrawal_realtime_data_repository.get_currency_quote(cash_conversion=cash_conversion_references)
    ...
    def _get_topic_name(self) -> str:
        return config("TRANSFERS_BETWEEN_DRIVE_WEALTH_AND_SINACOR_TOPIC")
