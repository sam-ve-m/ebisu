...
class UserBankAccountService:
    ...
    async def create_user_bank_accounts(
        ...
        is_bank_account_from_user = await bank_account_repository.is_user_bank_account_from_client(
        ...
        bank_account_exists_and_is_activated = await bank_account_repository.existing_user_bank_account_and_is_activated(
            unique_id=unique_id, bank_account=bank_account
        )
        ...
        create_account_response = {"message": "Created"}
    ...
    async def update_user_bank_account(
        ...
        user_bank_account_id_exists = await bank_account_repository.user_bank_account_id_exists(
            unique_id=unique_id, bank_account_id=bank_account_id
        )
        if not user_bank_account_id_exists:
            raise BadRequestError("common.register_not_exists")

        user_bank_account_was_updated = await bank_account_repository.update_registered_user_bank_accounts(
            unique_id=unique_id, bank_account=bank_account
        )
        if not user_bank_account_was_updated:
            raise InternalServerError("common.process_issue")

        update_bank_account_response = {"message": "Updated"}
    ...
    async def delete_user_bank_account(
        ...
        user_bank_account_was_soft_deleted = await bank_account_repository.delete_registered_user_bank_accounts(
            unique_id=unique_id, bank_account=bank_account
        )
        if not user_bank_account_was_soft_deleted:
            raise InternalServerError("common.process_issue")

        delete_bank_account_response = {"message": "Deleted"}
