...
class GetStatement:
    ...
    async def get_br_bank_statement(
        cls, jwt_data: dict, statement: GetBrStatement
    ) -> BrStatementResponse:
    ...
    async def get_us_bank_statement(
        jwt_data: dict, statement: GetUsStatement
    ) -> UsStatementResponse:
        ...
        from_date, to_date, offset = await get_rage_date_and_offset(
            unique_id=unique_id, requested_offset=statement.offset
        )
        ...
        transactions = await get_transactions(transaction_request=transaction_request)
    ...
    async def __get_range_date_and_offset(
        unique_id: str, requested_offset: int
        ...
        requested_raw_offset = GetStatement.__get_offset(
            requested_offset=requested_offset, from_date=from_raw_date
        )
