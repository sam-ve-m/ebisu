...
class ExchangeOperationsService:
    ...
    async def get_service_response_to_save_exchange_operations(
        ...
        exchange_model = ExchangeOperationsModel(exchange_operations_dto=exchange_operations_dto)
        exchange_data_was_dully_inserted = await cls.exchange_operations_repository_instance.\
            save_user_exchange_operations(
