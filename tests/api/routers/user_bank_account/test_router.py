...
@patch.object(UserBankAccountService, 'get_user_bank_accounts', return_value=get_accounts_dummy)
async def test_when_getting_the_bank_account_with_a_valid_jwt_then_return_the_bank_accounts(
    ...
    response = await UserBankAccountsRouter.get_user_bank_accounts(
        request=MagicMock(scope=scope_correct_stub, headers=MagicMock(raw=x_thebes_bank_tuple)))
    assert response == get_accounts_dummy
    assert isinstance(response, dict)
    assert response.get("bank_accounts")[1].get("id") == "15f11e59-4d44-4939-9bf0-25c1501e3b5d"


@pytest.mark.asyncio
@patch.object(JwtService, 'get_thebes_answer_from_request', return_value=payload_data_dummy)
@patch.object(UserBankAccountService, 'get_user_bank_accounts', return_value={"bank_accounts": []})
...
async def test_when_sending_the_wrong_payload_jwt_invalid_to_get_balance_then_raise_unauthorized_error():
    ...
    with pytest.raises(UnauthorizedError):
        await UserBankAccountsRouter.get_user_bank_accounts(request=MagicMock(scope=scope_wrong_stub))

# create bank account router
@pytest.mark.asyncio
@patch.object(JwtService, 'get_thebes_answer_from_request', return_value=jwt_data_to_create_account)
@patch.object(UserBankAccountService, 'create_user_bank_accounts', return_value={'message': 'Created'})
async def test_when_getting_the_bank_account_with_a_valid_jwt_then_return_the_bank_accounts(
    ...
    response = await UserBankAccountsRouter.create_user_bank_accounts(
        request=MagicMock(scope=scope_correct_stub, headers=MagicMock(raw=[x_thebes_bank_tuple, x_mist_bank_tuple])),
...
async def test_when_sending_an_invalid_jwt_to_create_account_then_raise_unauthorized_error():
    ...
    with pytest.raises(UnauthorizedError):
        await UserBankAccountsRouter.create_user_bank_accounts(
            request=MagicMock(scope=scope_correct_stub,
                              headers=MagicMock(raw=[scope_stub])),
            create_bank_account=CreateUserBankAccount(**create_account_stub))
...
async def test_when_sending_an_invalid_param_of_model_to_create_account_then_raise_internal_server_error():
    ...
    with pytest.raises(BaseException):
        await UserBankAccountsRouter.create_user_bank_accounts(
            request=MagicMock(scope=scope_correct_stub,
                              headers=MagicMock(
                                  raw=x_thebes_bank_tuple)),
            create_bank_account=None)


# update user bank account
@pytest.mark.asyncio
@patch.object(JwtService, 'get_thebes_answer_from_request', return_value=jwt_with_bank_account_to_update)
@patch.object(UserBankAccountService, 'update_user_bank_account', return_value={'message': 'Updated'})
async def test_when_getting_the_bank_account_with_a_valid_jwt_then_return_the_bank_accounts(
...
@pytest.mark.asyncio
@patch.object(JwtService, 'get_thebes_answer_from_request', return_value=jwt_with_bank_account_to_delete)
@patch.object(UserBankAccountService, 'delete_user_bank_account', return_value={'message': 'Deleted'})
async def test_when_getting_the_bank_account_with_a_valid_jwt_then_return_the_bank_accounts(
    ...
    response = await UserBankAccountsRouter.delete_bank_account(
        request=MagicMock(scope=scope_correct_stub, headers=MagicMock(raw=[x_thebes_bank_tuple, x_mist_bank_tuple])),
        delete_bank_account=DeleteUsersBankAccount(**delete_router_account_stub))
