...
@patch(
    "src.services.get_earnings.get_client_earnings.EarningsService.normalize_earnings",
    return_value=earnings_dummy_response,
)
def test_when_a_request_of_normalization_is_sent_then_return_the_normalized_data(
    mock_normalize_earnings,
):
    normalized_response = EarningsService.normalize_earnings(
        client_earnings=normalize_earnings_dummy_request
    )
...
async def test_when_a_valid_request_is_sent_then_return_the_earnings_expected_response(
    mock_get_service_response,
):
    earnings_response = await EarningsService.get_service_response(
        earnings=earnings_dummy_br
    )
...
@patch("src.services.get_earnings.get_client_earnings.earnings_regions")
async def test_when_invalid_request_is_sent_then_return_an_empty_object(
    mock_earnings_region, mock_build_query_earnings, mock_get_data
):
    mock_earnings_region.__getitem__ = MagicMock(return_value=GetBrEarnings)
    earnings_response = await EarningsService.get_service_response(
        earnings=MagicMock(symbol="PETR4", timestamp=1649611840000, offset=0, limit=1)
    )
...
@pytest.mark.asyncio
@patch(
    "src.repositories.base_repositories.oracle.repository.OracleBaseRepository.get_data",
    return_value=10000.41,
)
async def test_when_sending_the_query_then_return_the_expected_value_of_earnings(
    mock_get_data,
):
    query_dummy = query_dummy_earnings
    EarningsService.oracle_earnings_singleton_instance = StubOracleRepository
    response = EarningsService.oracle_earnings_singleton_instance.get_data(
        sql=query_dummy
    )
...
async def test_when_sending_an_invalid_query_then_return_the_expected_value(
    mock_get_data,
):
    query_dummy = None
    EarningsService.oracle_earnings_singleton_instance = StubOracleRepository
