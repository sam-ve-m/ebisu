...
@pytest.mark.asyncio
@patch.object(
    UserBankAccountRepository, "get_registered_user_bank_accounts", return_value={"bank_accounts": []}
)
...
async def test_get_user_account_when_jwt_param_is_not_valid_then_raise_error_as_expected():
    with pytest.raises(TypeError):
        await UserBankAccountService.get_user_bank_accounts(
