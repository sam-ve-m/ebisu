...
earnings_client_response_stub = {'payable_earnings': [{'client_code': 111,
                       'net_price': 0.0,
                       'trade_code': 'ALPA4',
                       'trade_history': 'LC03C00006TRANSFERENCIA CRÉDITO',
                       'trade_type': None,
                       'transaction_amount': 300.0,
                       'transaction_date': '2022-05-10'}], 'record_date_earnings': []}

client_earnings_stub = [{'COD_CLI': 111, 'DESC_HIST_MVTO': 'LC03C00006TRANSFERENCIA CRÉDITO', 'DESC_RESU_TIPO_MVTO': 'TRANSFERENCIA', 'COD_NEG': 'ALPA4', 'QTDE_MVTO': 300.0, 'PREC_LQDO': 0.0, 'DATA_MVTO': "2022-05-10"}]
client_earnings_2_stub = {}
normalize_param_stub = {'COD_CLI': 111, 'DESC_HIST_MVTO': 'LC03C00006TRANSFERENCIA CRÉDITO', 'DESC_RESU_TIPO_MVTO': 'TRANSFERENCIA', 'COD_NEG': 'ALPA4', 'QTDE_MVTO': 300.0, 'PREC_LQDO': 0.0, 'DATA_MVTO': "2022-05-10"}
normalized_stub = {'client_code': 111, 'trade_history': 'LC03C00006TRANSFERENCIA CRÉDITO', 'trade_type': None, 'trade_code': 'ALPA4', 'transaction_amount': 300.0, 'net_price': 0.0, 'transaction_date': "2022-05-10"}
...
def test_when_sending_the_right_params_to_get_account_by_region_then_return_the_expected():
    response = EarningsFromClient.get_account_by_region(
        portfolios={'bovespa_account': '000000014-6', 'bmf_account': '14'},
        region="BR"
    )
    assert response == "14"
...
@patch("src.services.earnings_from_client.get_earnings_from_client.earnings_client_region")
@patch.object(GetBrEarningsDetails, 'build_query_payable_earnings', return_value=MagicMock)
@patch.object(GetBrEarningsDetails, 'build_query_record_date_earnings', return_value=MagicMock)
